...
{
#ifdef ANTON
	static int esc_down = false;
	static int enter_down = false;
	static int fwd_down = false;
	static int bk_down = false;
	static int lft_down = false;
	static int rgt_down = false;
	static int y_down = false;
	static int lctrl_down = false;
	static int space_down = false;
	...
	if (GLFW_RELEASE == glfwGetKey (window, GLFW_KEY_ESCAPE)) {
		if (esc_down) {
			event_t event;
			event.type = ev_keyup;
			event.data1 = KEY_ESCAPE;
	...
	if (GLFW_RELEASE == glfwGetKey (window, GLFW_KEY_DOWN)) {
		if (bk_down) {
			event_t event;
			event.type = ev_keyup;
			event.data1 = KEY_DOWNARROW;
	...
	if (GLFW_PRESS == glfwGetKey (window, 'Y')) {
		if (!y_down) {
			event_t event;
			event.type = ev_keydown;
			event.data1 = 'y';
	...
	if (GLFW_RELEASE == glfwGetKey (window, 'Y')) {
		if (y_down) {
			event_t event;
			event.type = ev_keyup;
			event.data1 = 'y';
	...
	if (GLFW_RELEASE == glfwGetKey (window, GLFW_KEY_LEFT_CONTROL)) {
		if (lctrl_down) {
			event_t event;
			event.type = ev_keyup;
			event.data1 = KEY_RCTRL;
...
void I_FinishUpdate (void)
{
...
#ifdef ANTON
	for (i = 0; i < 2 ;i++) {
		olineptrs[i] = (unsigned int *) &image_data[i * X_width];
	}
#else
	for (i=0 ; i<2 ; i++)
  ...
  Expand4 ((unsigned *)(screens[0]), (double *) (image_data));
#else
	void Expand4(unsigned *, double *);
  	Expand4 ((unsigned *)(screens[0]), (double *) (image->data));
		...
		int c_i = 0;
		int id;
		for (id = 0; id < 320 * 200; id++) {
			int v = (int)image_data[id];
			char r = p_red[v];
			char g = p_green[v];
			char b = p_blue[v];
			corrected[c_i++] = r;
		...
		// avoid any read-write during drawing (shouldn't happen)
		glFlush ();
		glFinish ();
...
void I_InitGraphics(void)
{
	...
	else if (n==6)
	{
		...
		window = glfwCreateWindow (g_gl_width, g_gl_height, "crongdor", mon, NULL);
    ...
    if (multiply == 1) {
#ifdef ANTON
	screens[0] = (unsigned char *) (image_data);
#else
  ...
  double*	xline )
{
	...
	do
	{
	    ...
	    dpixel = *(double *)( (long int)exp + ( (fourpixels&0xffff0000)>>13) );
	    xline[0] = dpixel;
	    xline[160] = dpixel;
	    xline[320] = dpixel;
	    xline[480] = dpixel;
			
	    dpixel = *(double *)( (long int)exp + ( (fourpixels&0xffff)<<3 ) );
	    xline[1] = dpixel;
	    xline[161] = dpixel;
	    xline[321] = dpixel;
	    xline[481] = dpixel;
	    ...
	    dpixel = *(double *)( (long int)exp + ( (fourpixels&0xffff0000)>>13) ); // ANTON
	    xline[4] = dpixel;
	    xline[164] = dpixel;
	    xline[324] = dpixel;
	    xline[484] = dpixel;
			
	    dpixel = *(double *)( (long int)exp + ( (fourpixels&0xffff)<<3 ) ); // ANTON
	    xline[5] = dpixel;
	    xline[165] = dpixel;
	    xline[325] = dpixel;
	    xline[485] = dpixel;
